...
def extract_opinions_from_file(kaf_filename, tag_filename,out_filename,map_distrib_opi_ents, overall_map):
    ...
    for token_obj in knaf_obj.get_tokens():
        ...
        whole_text += token_obj.get_text().lower()+' '
    ...
    for anot in annotations:
        ...
        if aspect_id is not None and aspect_id != '0':
            if aspect_id not in aspects:
                aspects[aspect_id] = []
            aspects[aspect_id].append((aspect_label,wid,anot.token))
            aspect_for_wid[wid] = aspect_label
    ...
    for opi_id, list_tokens in opinions.items():
        ...
        if not opinion_is_ok and crossing_ents:
            ...
            for ent_type, list_wid_token in expressions:
                str_exp = (' '.join(token for wid,token in list_wid_token)).lower()
                
                if str_exp not in map_for_this_file:
                    map_for_this_file[str_exp] = whole_text.count(str_exp)
                map_distrib_opi_ents['expression'][str_exp.lower()+'#'+ent_type]+=1
            ...
            for list_wid_token_tar in targets:
                str_tar = (' '.join(token for wid,token in list_wid_token_tar)).lower()
                if str_tar not in map_for_this_file:
                    map_for_this_file[str_tar] = whole_text.count(str_tar)
                str_aspects = ' '.join(list(set( aspect_for_wid.get(wid,'') for wid, token in list_wid_token_tar if wid in aspect_for_wid)))
                
                if len(str_aspects) != 0:
                    str_tar+='#'+str_aspects
                map_distrib_opi_ents['target'][str_tar.lower()]+=1
                        
            for list_wif_token_hol in holders:
                str_hol = (' '.join(token for wid,token in list_wif_token_hol)).lower()
                map_distrib_opi_ents['holder'][str_tar.lower()]+=1
    ...
    for s, c in map_for_this_file.items():
        overall_map[s]+=c
...
def extract_opinions(tag_folder,kaf_folder,out_folder):
    ...
    for kaf_filename in glob.glob(kaf_folder+'/*.kaf'):
        ...
        if not os.path.exists(tag_filename):
            ...
            num_tag_files += 1
            num_scope_opis, num_skipped, num_triples,  num_exps, num_tars, num_hols, num_sents, num_sents_opis = extract_opinions_from_file(kaf_filename,tag_filename,out_filename,map_distrib_opi_ents,overall_map)
    ...
    dis_exp = defaultdict(int)
    for a,c in map_distrib_opi_ents['expression'].items():
        p=a.rfind('#')
        aux = a[p+1:]
        dis_exp[aux]+=1
    for t,c in dis_exp.items():
        print '    ',t,'-->',c
    ...
    for n,(s, freq) in enumerate(v[:this_max]):
        ...
        if p!=-1:
            ...
            saux = s
        print '    Exp '+str(n)+'/'+str(len(v)), s.encode('utf-8')
        print '        Annotated as expression:',freq
        print '        Overall frequency:',overall_map[saux]
        print '        % as expression:', round(freq*100.0/total_exps,2),'%'
    ...
    for n,(s, freq) in enumerate(v[:this_max]):
        ...
        if p!=-1:
            ...
            saux = s
        print '    Tar '+str(n)+'/'+str(len(v)), s.encode('utf-8')
        print '        Annotated as target:',freq
        print '        Overall frequency:',overall_map[saux]
        print '        % as target:', round(freq*100.0/total_tars,2),'%'
